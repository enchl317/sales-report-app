...
const InventoryCountDetailPage: React.FC = () => {
    ...
    const fetchInventoryCountDetail = async () => {
      try {
        ...
        if (typeof id === 'string' && isNaN(Number(id))) {
          response = await fetch(`/api/inventory-count/${encodeURIComponent(id)}`);
        } else {
          // 如果是纯数字，则按ID获取
          response = await fetch(`/api/inventory-count/${id}`);
        }
        
        result = await response.json();

        if (result.success) {
          setInventoryCount(result.inventoryCount);
          setDetails(result.details);
        } else {
          setError(result.message || '获取盘点详情失败');
        }
      } catch (err) {
        setError('获取盘点详情失败');
        console.error('Error fetching inventory count detail:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!inventoryCount) {
    return (
      <div className="min-h-screen bg-gray-50 py-8">
        <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-6">
          <div className="text-gray-500">未找到盘点记录</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-6">
        <h2 className="text-2xl font-bold text-center mb-6">库存盘点详情</h2>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              type="text"
              value={inventoryCount.documentId}
              readOnly
          ...
          <div>
            ...
            <input
              type="text"
              value={inventoryCount.storeName}
              readOnly
              className="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-100"
            />
          </div>
          ...
          <div>
            ...
            <input
              type="text"
              value={formatDate(inventoryCount.createdDate)}
              readOnly
              className="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-100"
            />
          </div>
...
export default InventoryCountDetailPage;